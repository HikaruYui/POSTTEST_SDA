...
void tampilAntrian() {
    ...
    Tiket* ptr = queueTiket;

    for (int i = front; i <= rear; i++) {
        table.add_row({
            to_string(i - front + 1),
            (ptr + i)->namaPenumpang,
            (ptr + i)->keretaDipilih.nama,
            (ptr + i)->keretaDipilih.asal,
            (ptr + i)->keretaDipilih.tujuan,
            "Rp" + to_string((ptr + i)->keretaDipilih.harga)
        });
    }
...
void enqueue(Tiket value) {
    ...
    rear++;
    queueTiket[rear] = value;
    cout << value.namaPenumpang << " masuk ke antrian pembelian tiket.\n";
}
